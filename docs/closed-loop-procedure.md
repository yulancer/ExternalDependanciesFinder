...
| Утилита | Назначение |
|---|---|
| `ExternalDependenciesFinder` | Формирует проект со всеми пакетами, включая транзитивные зависимости исходного списка. |
| `ExternalDependenciesFinder.NugetAccessChecker` | Проверяет доступность пакетов внутри закрытого контура и формирует список недоступных пакетов. |
...
Закрытый контур
    |
    | 3. ExternalDependenciesFinder.NugetAccessChecker
    v
...
Запустите `ExternalDependenciesFinder`, передав путь к исходному проекту:

```bash
dotnet run --project path/to/ExternalDependenciesFinder.csproj -- "path/to/InitialProjectForUpdate.csproj"
```

Или, если утилита уже собрана:

```bash
ExternalDependenciesFinder.exe "path/to/InitialProjectForUpdate.csproj"
```
...
```text
ExternalDependenciesFinder/ExternalDependenciesFinder/bin/Debug/net8.0/NugetUsingSolution/NugetUsingSolution.sln
```
...
```bash
dotnet run --project path/to/ExternalDependenciesFinder.NugetAccessChecker.csproj -- "path/to/NugetUsingProject.csproj"
```

Или, если утилита уже собрана:

```bash
ExternalDependenciesFinder.NugetAccessChecker.exe "path/to/NugetUsingProject.csproj"
```
...
Результатом работы `ExternalDependenciesFinder.NugetAccessChecker` является файл:
...
repo-root/
  ...
  tools/
    ...
    ExternalDependenciesFinder.NugetAccessChecker/
    TestProjectCreator/
  output/
    NugetUsingSolution/
    TestSolution/
...
dotnet run --project tools/ExternalDependenciesFinder/ExternalDependenciesFinder.csproj -- "InitialProjectForUpdate.csproj"
```
...
```bash
dotnet run --project tools/ExternalDependenciesFinder.NugetAccessChecker/ExternalDependenciesFinder.NugetAccessChecker.csproj -- "NugetUsingProject.csproj"
```
...
- [ ] `ExternalDependenciesFinder` сформировал проект с полным списком зависимостей.
- [ ] `NugetUsingProject.csproj` передан внутрь закрытого контура.
- [ ] В закрытом контуре запущен `ExternalDependenciesFinder.NugetAccessChecker`.
